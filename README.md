...
- **全屏有两个目标**：整个网页（房间头部）与队伍分配区（工具栏）。都用 Fullscreen API，
  没有做「另开一个页面」：房间状态在房主的内存里、成员各自在浏览器里，另开一个页面就得再
  同步一份状态，还得处理哪个窗口说了算。队伍区在不支持元素级全屏的浏览器上（如 iPad Safari）
  退回固定定位的伪全屏；网页全屏没有退化方案，不支持时按钮不出现。
