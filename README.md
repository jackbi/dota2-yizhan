...
**但它不总是因为 IP 真的变了。** 实测过一次连续二十分钟的 403，期间出口 IP 稳定在同一个值
（每 12 秒采一次，14 次全一样，两条不同的回显服务也对得上），之后又自己恢复。token 本身不带
IP——解出来只有 `Subject` / `SteamId` / `APIUser` / `exp`，所以绑定状态在 STRATZ 那侧，
从本地看不出它认的是哪个 IP。能做的只有：出口保持稳定（别让代理按延迟自动切节点），
并且**只在一个地方用这个 token**——同一个 token 在线上部署和本地开发同时用，是最容易触发它的用法。

取数层把这种情况按「上游故障」处理，页面给可重试的提示，而不是当成「这个玩家没有数据」。
