...
- **大厅是星形，不是全网状。** 大厅访客用 `passive: true` 加入（只听不播，访客之间因此
  不会互建连接），房主以正常身份广播自己的房间，每 15 秒一次，45 秒没重播就从列表里消失。
  不做这一步的话，N 个访客会互相建 N×(N-1)/2 条连接。
...
- **一个房间 12 人封顶**（`ROOM_MAX_MEMBERS`）。P2P 全网状，再多连接数会明显拖慢浏览器。
- **不用拖拽分队。** 归队是每张成员卡上的下拉框：拖拽在触屏和键盘上都要另做一套等价操作，
  而下拉天然三端可用（同「分屏直播」里「拖拽必须有等价操作」的结论）。
