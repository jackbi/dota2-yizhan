...
- **对局复盘与地图回放**（`/replay/<比赛 id>`）：经济与经验差曲线、STRATZ 胜率曲线、三路结果、
  建筑倒塌时间轴与视野统计（插眼 / 反眼 / 到期分开算）。另有十人对抗明细表（伤害 / 塔伤 / 治疗、
  6 主物品 + 3 背包 + 中立物品）与十名英雄逐秒位置的地图回放（可拖时间轴、含眼位与肉山）。
  明细与面板跟着页面渲染，数百 KB 的轨迹等读者点了才取；没有录像的对局只显示前两块，并说明原因
